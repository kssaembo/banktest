...
export function DonutCard({ title, data, onSelect, centerLabel = '전체' }: { title: string; data: ChartSlice[]; onSelect?: (item: ChartSlice) => void; centerLabel?: string }) {
  ...
  return <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm">
    ...
    <div className="relative h-56">
      <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={renderData} dataKey="value" nameKey="name" innerRadius={56} outerRadius={83} paddingAngle={3} onClick={(entry: any) => onSelect?.(entry)}>{renderData.map((_, index) => <Cell key={index} fill={usable.length ? CHART_COLORS[index % CHART_COLORS.length] : '#e2e8f0'} className={onSelect ? 'cursor-pointer' : ''}/>)}</Pie><Tooltip contentStyle={tooltipStyle} formatter={(value: number, _name, item: any) => [usable.length ? `${Number(value).toLocaleString()} (${Math.round(Number(value) / Math.max(1,total) * 100)}%)` : '-', item.payload.name]} content={({active,payload})=>{if(!active||!payload?.[0])return null;const row=payload[0].payload as ChartSlice;return <div className="max-w-64 rounded-2xl border bg-white p-3 text-xs shadow-xl"><p className="font-black text-slate-800">{row.name}: {row.value.toLocaleString()}</p>{row.details?.length?<p className="mt-2 leading-5 text-slate-600">{row.details.join(', ')}</p>:null}</div>;}}/></PieChart></ResponsiveContainer>
...
export function VerticalBars({ title, data, unit = '', onSelect }: { title: string; data: ChartSlice[]; unit?: string; onSelect?: (item: ChartSlice) => void }) {
  const sorted=[...data].sort((a,b)=>b.value-a.value);
  return <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm"><h3 className="font-black text-slate-800">{title}</h3><div className="mt-3 h-60 min-w-[420px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={sorted} margin={{top:10,right:8,left:-20,bottom:18}}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" tick={{fontSize:11,fontWeight:700}} interval={0}/><YAxis hide/><Tooltip contentStyle={tooltipStyle} formatter={(v:number)=>`${Number(v).toLocaleString()}${unit}`}/><Bar dataKey="value" radius={[12,12,3,3]} onClick={(entry:any)=>onSelect?.(entry)}>{sorted.map((_,i)=><Cell key={i} fill={CHART_COLORS[i%CHART_COLORS.length]} className={onSelect?'cursor-pointer':''}/>)}</Bar></BarChart></ResponsiveContainer></div></section>;
}
...
export function SegmentedBar({ title, rows, unit = '' }: { title: string; rows: ChartSlice[]; unit?: string }) {
  ...
  return <div className="rounded-2xl bg-slate-50 p-4"><h3 className="mb-3 font-black text-slate-800">{title}</h3><div className="flex h-12 overflow-hidden rounded-2xl bg-slate-200">{rows.filter(r=>r.value>0).map((row,i)=><div key={row.name} title={`${row.name}: ${row.value.toLocaleString()}${unit}`} className="group relative flex min-w-[4px] items-center justify-center text-[10px] font-black text-white" style={{width:`${row.value/Math.max(1,total)*100}%`,background:CHART_COLORS[i%CHART_COLORS.length]}}><span className="hidden px-1 group-hover:block">{row.name}</span></div>)}</div><div className="mt-3 flex flex-wrap gap-3">{rows.map((row,i)=><span key={row.name} className="text-xs font-bold text-slate-600"><i className="mr-1 inline-block h-2.5 w-2.5 rounded-full" style={{background:CHART_COLORS[i%CHART_COLORS.length]}}/>{row.name} {row.value.toLocaleString()}{unit}</span>)}</div></div>;
}
