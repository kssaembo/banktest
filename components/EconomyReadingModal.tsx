...
const orderedQuizOptions=(options:string[],lessonOrder:number,questionIndex:number)=>{
  const shift=(lessonOrder+questionIndex*2)%options.length;
  return options.map((label,originalIndex)=>({label,originalIndex})).slice(shift).concat(options.map((label,originalIndex)=>({label,originalIndex})).slice(0,shift));
};
    ...
    <section className="flex h-full w-full flex-col overflow-hidden bg-[#eef5ff] shadow-2xl md:h-[94vh] md:rounded-[36px]" onClick={e=>e.stopPropagation()}>
      ...
      {!lesson?<div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-8"><div className="mx-auto max-w-6xl">
        ...
        {step<3?<LessonSlide lesson={lesson} step={step} revealed={revealed} setRevealed={setRevealed} challengeChoice={challengeChoice} setChallengeChoice={setChallengeChoice} resetStep={resetStep}/>:<div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-5xl rounded-[32px] bg-white p-5 shadow-lg md:p-8">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">{lesson.quiz.map((item,index)=>{const displayed=orderedQuizOptions(item.options,lesson.order,index);const correctDisplayIndex=displayed.findIndex(option=>option.originalIndex===item.answer);return <article key={item.question} className="rounded-[24px] border border-blue-100 bg-blue-50/40 p-5"><span className="text-xs font-black text-blue-600">문제 {index+1}</span><h4 className="mt-2 min-h-14 font-black leading-6 text-slate-800">{item.question}</h4><div className="mt-3 space-y-2">{displayed.map((option,displayIndex)=><button key={option.originalIndex} disabled={Boolean(result)} onClick={()=>setQuizAnswers(values=>values.map((value,i)=>i===index?option.originalIndex:value))} className={`w-full rounded-xl border-2 px-3 py-2.5 text-left text-sm font-bold ${quizAnswers[index]===option.originalIndex?'border-blue-500 bg-white text-blue-700':'border-transparent bg-white text-slate-600'}`}>{displayIndex+1}. {option.label}</button>)}</div>{result&&<p className={`mt-3 text-xs font-bold ${quizAnswers[index]===item.answer?'text-emerald-700':'text-red-600'}`}>{quizAnswers[index]===item.answer?'정답! ':`정답은 ${correctDisplayIndex+1}번. `}{item.explain}</p>}</article>})}</div>
