...
import { announceReward, getRewardSettings, submitReadingLearning } from '../services/learningAddons';
...
export const EconomyReadingModal: React.FC<Props> = ({ isOpen, onClose, user, onBalanceChanged }) => {
  ...
  const openLesson=(id:string)=>{setSelectedId(id);setQuizAnswers([null,null,null]);setResult(null);resetStep(0)};
  ...
  const submitQuiz=async()=>{
    ...
    try{const reward=await submitReadingLearning(user.userId,lesson.id,answers);setResult({passed:true,correct,...reward});announceReward(reward.reward,unit);onBalanceChanged?.();}
  ...
  return <div className="fixed inset-0 z-[220] flex items-center justify-center bg-slate-950/70 p-0 backdrop-blur-md md:p-4" onClick={onClose}>
    <section className="flex h-full w-full flex-col overflow-hidden bg-[#eef5ff] shadow-2xl md:h-[94vh] md:rounded-[36px]" onClick={e=>e.stopPropagation()}>
      ...
      {!lesson?<div className="min-h-0 flex-1 overflow-y-auto p-5 md:p-8"><div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{economyReadingLessons.map(item=><button key={item.id} onClick={()=>openLesson(item.id)} className="group overflow-hidden rounded-[26px] border border-white bg-white text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg"><div className="relative h-32 overflow-hidden bg-slate-100"><img src={`/economy_reading/${item.file}`} alt="" className="h-full w-full object-cover opacity-75 transition group-hover:scale-105"/><div className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-30`}/><span className="absolute left-3 top-3 rounded-full bg-slate-950/70 px-3 py-1 text-xs font-black text-white">CH.{item.order}</span></div><div className="p-4"><h4 className="text-lg font-black text-slate-900">{item.title}</h4><p className="mt-2 line-clamp-2 text-xs font-semibold leading-5 text-slate-500">{item.summary}</p><span className="mt-4 block text-xs font-black text-blue-600">탐험 시작 →</span></div></button>)}</div>
...
function LessonSlide({lesson,step,revealed,setRevealed,challengeChoice,setChallengeChoice,resetStep}:any){
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6"><div className={`mx-auto grid min-h-full max-w-6xl gap-5 rounded-[32px] bg-gradient-to-br ${lesson.color} p-5 text-white shadow-xl md:grid-cols-[.78fr_1.22fr] md:p-8`}>
    <div className="flex flex-col justify-between rounded-[26px] bg-white/15 p-6 backdrop-blur-sm"><div><span className="rounded-full bg-white/20 px-3 py-1 text-xs font-black">SLIDE {step+1} / 3</span><div className="mt-6 text-7xl">{current.emoji}</div><h3 className="mt-5 text-3xl font-black md:text-4xl">{current.title}</h3><p className="mt-4 text-sm font-semibold leading-7 text-white/90 md:text-base">{current.lead}</p></div><p className="mt-6 text-xs font-black text-white/80">카드를 눌러 핵심 내용을 펼쳐보세요.</p></div>
    <div className="flex flex-col gap-4"><div className="grid gap-3 sm:grid-cols-3">{current.facts.map((fact:any,index:number)=>{const open=revealed.includes(index);return <button key={fact.label} onClick={()=>setRevealed((values:number[])=>values.includes(index)?values:[...values,index])} className={`min-h-36 rounded-[22px] border-2 p-4 text-left transition ${open?'border-white bg-white text-slate-800 shadow-lg':'border-white/30 bg-white/15 text-white hover:bg-white/25'}`}><span className="text-3xl">{fact.emoji}</span><strong className="mt-3 block">{fact.label}</strong><p className={`mt-2 text-xs font-semibold leading-5 ${open?'text-slate-600':'text-white/70'}`}>{open?fact.detail:'눌러서 알아보기'}</p></button>})}</div>
      ...
      <div className="mt-auto flex justify-between gap-3"><button disabled={step===0} onClick={()=>resetStep(step-1)} className="rounded-2xl bg-white/20 px-5 py-3 font-black disabled:opacity-30">← 이전</button><button disabled={challengeChoice===null} onClick={()=>resetStep(step+1)} className="rounded-2xl bg-white px-6 py-3 font-black text-blue-700 shadow disabled:opacity-40">{step===2?'확인 문제 풀기':'다음 슬라이드'} →</button></div>
    </div>
  </div></div>;
}
