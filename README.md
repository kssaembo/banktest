...
003까지 적용한 기존 설치는 `supabase/migrations/202609160004_archive_revision.sql`을 한 번 실행한 뒤 프런트엔드를 업데이트합니다. 새 설치는 001 → 002 → 003 → 004 순서입니다. 이미 적용한 파일은 반복 실행하지 마세요. 기존 입력 기록과 사진은 유지됩니다.
...
1. SQL Editor에서 `supabase/migrations/202609150001_initial.sql`부터 migrations 폴더의 001 → 002 → 003 → 004를 순서대로 실행합니다. 각 파일은 한 번만 실행합니다.
...
- 교사 프로젝트 옆 ×, 학생 닉네임 행의 ×로 삭제합니다. 확인창 후 목록에서 숨기며 학생 접근을 차단합니다. 기록·사진은 보관하고 ‘삭제 항목 보기·복구’에서 되돌릴 수 있습니다. 영구 삭제가 아니므로 저장 공간은 유지됩니다.
- 삭제한 닉네임으로 새 참가가 가능하지만 기존 기록은 자동으로 합쳐지지 않습니다. 같은 닉네임이 다시 사용 중이면 복구 전에 현재 학생을 삭제해야 합니다.
- 입력이 멈춘 뒤 약 2.5초 후 자동 저장합니다. 작성 중 텍스트는 기기 localStorage에 임시 보관하고 서버 저장 성공 후 제거합니다. 체험 모드는 제외합니다. 저장 실패 시 입력을 유지하며 ‘임시본 다운로드’로 JSON 보관이 가능합니다.
- 재접속 시 임시본 복원 여부를 선택합니다. 서버 버전과 다르거나 제출 완료이면 복원을 차단하고 다운로드만 허용합니다. 서로 다른 기기에서 같은 과거 버전으로 저장하면 두 번째 저장은 거절합니다.
- 이 임시 보관은 DB·사진 전체 백업을 대신하지 않습니다. 브라우저 데이터 삭제 시 임시본은 사라집니다. 사진 자체는 Supabase Storage에만 저장됩니다. 로그아웃 시 미저장 변경을 먼저 저장하며 실패하면 로그아웃하지 않습니다.
- 도시·관광지는 추가 버튼으로 여러 항목을 등록합니다. 기존 city 문자열은 그대로 첫 항목으로 유지하고 여러 항목은 줄바꿈으로 저장하므로 기존 기록을 변환·삭제하지 않습니다.
- 포트폴리오는 표지+출발 / 해외 여행지 / 도착+여권으로 구성합니다. 긴 기록·많은 준비물·스탬프는 필요한 만큼 다음 페이지로 이어집니다. 출력 내용이 잘리지 않도록 무조건 한 장으로 축소하지 않습니다.
- 날짜·파일명·URL 제거: 브라우저 인쇄 창 → 설정 더보기 → 머리글과 바닥글 끄기. 웹사이트가 이 브라우저 옵션을 강제 변경하지는 않습니다. A4, 배경 그래픽 켜기를 권장합니다. [인쇄 API](https://developer.mozilla.org/en-US/docs/Web/API/Window/print).
- 서비스 평가와 개선 우선순위는 SERVICE_REVIEW.md를 참고하세요.
